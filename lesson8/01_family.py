...
class House:
    ...
    def __str__(self):
        return 'В доме еды осталось {}, денег осталось {}, кол-во грязи {}, кошачего корма осталось {}'.format(
            self.food, self.money, self.dirt, self.catfood)
...
class Man:
    ...
    def pet_cat(self):
        self.happy += 5
        cprint('{} - произведено глажение кота'.format(self.name), color='green')
...
class Wife(Man):
    ...
    def act(self):
        ...
        if self.fullness < 15:
            ...
            self.buy_fur_coat()
        elif (self.house.food < 60) or (self.house.catfood < 10):
            self.shopping()
    ...
    def shopping(self):
        if self.house.money >= 90:
            ...
            if self.house.catfood > self.house.food:
                self.house.money -= 90
                self.house.food += 70
                self.house.catfood += 20
            else:
                self.house.money -= 90
                self.house.food += 40
...
for cat in cats:
    cat.go_to_the_house(house=home)
